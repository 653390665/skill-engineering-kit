# Manifest

## Root files

- `SKILL.md` — main activation and workflow rules.
- `README.md` — full overview.
- `QUICK_START.md` — fastest usage guide.
- `MANIFEST.md` — file inventory.
- `QUALITY_TESTS.md` — expected behavior tests for this skill.

## Prompts

- `prompts/00_mode_router.md`
- `prompts/01_new_skill_planning.md`
- `prompts/02_existing_skill_audit.md`
- `prompts/03_pressure_test_preparation.md`
- `prompts/04_result_interpretation.md`
- `prompts/05_release_gate.md`
- `prompts/06_toolchain_workflow.md`
- `prompts/07_yao_bridge.md`

## Templates

- `templates/skill_brief.md`
- `templates/architecture_decision.md`
- `templates/trigger_contract.md`
- `templates/output_contract.md`
- `templates/quality_test_plan.md`
- `templates/standard_audit_report.md`
- `templates/pressure_test_brief.md`
- `templates/optimization_report.md`
- `templates/release_gate.md`
- `templates/regression_log.md`
- `templates/skill_ir.json`

## References

- `references/usage_principles.md`
- `references/tool_responsibility_matrix.md`
- `references/standard_vs_pressure_testing.md`
- `references/priority_taxonomy.md`
- `references/yao-bridge.md`

## Scripts

- `scripts/lint_skill_package.py` — static package lint.
- `scripts/scaffold_report.py` — create report skeletons from a target skill path.
- `scripts/build_clean_install.py` — optional clean package builder.
- `scripts/validate_ir.py` — validate skill-ir.json against Skill IR 2.0 required fields.

## Examples

- `examples/example_new_skill_workflow.md`
- `examples/example_existing_skill_audit.md`
- `examples/example_pressure_interpretation.md`
